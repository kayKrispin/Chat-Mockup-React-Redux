...
export  function fetchUser(credentials) {
  ...
  const { password } = credentials;
    ...
    .then(res => { const user = res.data; return user})
    .then(user => {const token=user.token; store.dispatch(saveUser(user)); localStorage.setItem('userName', user); return token})
...
export function fetchUsers(){
  return Api.get(FETCH_USERS_URL)
  .then(res=>res.data);
}
